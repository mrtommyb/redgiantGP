...
class transitemcee_koi2133(transitemcee_rv.transitemcee_rv):
    ...
    def get_guess(self,nwalkers):
        ...
        #lcjitter
        #a refector to natural log
        #start,stop = -25, 1
        #p0[...,-2] = truncnorm.rvs(start,stop,
        #    loc=-1,scale=np.log(0.1*np.median(self.err)),size=nwalkers)

        start = (-25 - np.log(1.E-6)) / 1.
        stop = (1 - np.log(1.E-6)) / 1.
        p0[...,-2] = truncnorm.rvs(
            start,stop,
            loc=np.log(1.E-6), scale=1.,
            size=nwalkers)
        ...
        #    loc=-1,scale=np.log(0.1*np.median(self.rverr)),size=nwalkers)

        start = (-15 - np.log(1.E-1)) / 1.
        stop = (3 - np.log(1.E-1)) / 1.
        p0[...,-1] = truncnorm.rvs(
            start,stop,
            loc=np.log(1.E-1), scale=1.,
            size=nwalkers)
...
def logchi2_rv_phaseGP(fitsol,nplanets,rho_0,rho_0_unc,rho_prior,
    ...
    fitsol_model_calc = np.r_[fitsol[0:2],fitsol[4],fitsol[9:]] #cut out limb da
rkening
